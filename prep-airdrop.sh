...
if [[ ! -f "${CONFIG_FILE}" ]]; then
  ...
  # Use printf with %q to safely escape values, preventing command injection
  # when config.conf is later sourced
  generated_date="$(date '+%Y-%m-%d')"
  {
    printf '# config.conf - Generated by prep-airdrop.sh on %s\n\n' "${generated_date}"
    printf '# Server identity\n'
    printf 'SERVER_NAME=%q\n\n' "${SERVER_NAME}"
    printf '# 1Password configuration\n'
    printf 'ONEPASSWORD_VAULT=%q\n' "${ONEPASSWORD_VAULT}"
    printf 'ONEPASSWORD_APPLEID_ITEM=%q\n' "${ONEPASSWORD_APPLEID_ITEM}"
    printf 'ONEPASSWORD_TIMEMACHINE_ITEM=%q\n' "${ONEPASSWORD_TIMEMACHINE_ITEM}"
    printf 'SSH_KEY_ITEM=%q\n\n' "${SSH_KEY_ITEM:-SSH Keys}"
    printf '# Monitoring\n'
    ...
    printf 'TERMINAL_PROFILE_FILE=%q\n' "${TERMINAL_PROFILE_FILE:-}"
    printf 'USE_ITERM2=%q\n\n' "${USE_ITERM2:-false}"
    printf '# Optional overrides\n'
    printf 'HOSTNAME_OVERRIDE=%q\n' "${HOSTNAME_OVERRIDE:-}"
  } >"${CONFIG_FILE}"
